...
while [[ $CCOUNT -le $CREPS ]]; do
    ...
    if [ "x$USECMAKE" = "xTRUE" ]; then
        ...
        cd build-netcdf-c
        cmake ${HOME}/netcdf-c -DCMAKE_INSTALL_PREFIX=/usr -DENABLE_HDF4=ON -DENABLE_EXTRA_TESTS=OFF -DENABLE_TESTS=OFF -DENABLE_MMAP=ON -DBUILDNAME_PREFIX="docker$BITNESS-$USE_CC" -DBUILDNAME_SUFFIX="$CBRANCH" -DCMAKE_C_COMPILER=$USE_CC $COPTS -DENABLE_ERANGE_FILL=TRUE ; CHECKERR
        make clean

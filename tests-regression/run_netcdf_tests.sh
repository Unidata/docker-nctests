...
if [ -d "/netcdf-c" ]; then
    ...
    git clone https://www.github.com/Unidata/netcdf-c --branch $CBRANCH --depth=1 $CBRANCH
    mv $CBRANCH netcdf-c
fi
...
if [ "x$RUNF" == "xTRUE" ]; then
    ...
    if [ -d "/netcdf-fortran" ]; then
        ...
        git clone https://www.github.com/Unidata/netcdf-fortran --branch $FBRANCH --depth=1 $FBRANCH
...
if [ "${H4VER}" != "" ]; then
    ...
    H4ACOPT="--enable-hdf4"
    H4CMAKEOPT="-DNETCDF_ENABLE_HDF4=TRUE"
    ...
    if [ "x$USEAC" = "xTRUE" ]; then
        ...
        if [ "x$RUNC" == "xTRUE" ]; then
            ...
            if [ "x$DISTCHECK" == "xTRUE" ]; then
                DISTCHECK_CONFIGURE_FLAGS="${H4ACOPT} --enable-extra-tests --enable-mmap ${S3OPTS_AC} ${AC_COPTS}" make distcheck ; CHECKERR
            fi
